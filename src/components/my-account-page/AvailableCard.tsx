...
import { calculateCreditUsagePercentage } from "@/utils/generalFunctions";
...
const AvailableCard = () => {
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      if(userCredit.status === 200 && userCredit.data.length > 0) {
        const totalCredit = calculateCreditUsagePercentage(userCredit.data[0].totalCredit, userCredit.data[0].totalDeductedCredit)
        setUsagePercentage(totalCredit)
        setUserToken(userCredit.data[0].totalCredit)
  ...
  return (
    <div className="bg-neutral-800 p-6 rounded-lg lg:min-w-80 flex flex-col justify-between">
      ...
      <div>
        <h3 className="text-gray-400 mb-2">Available Tokens</h3>
        <div className="text-3xl font-bold mb-4">{userToken}</div>
        <div className="w-full bg-red h-2 rounded-full">
          <div className="bg-gray h-2 rounded-full transition-all ease-in-out duration-500" style={{width: `${usagePercentage}%`}}></div>
        </div>
      </div>
    </div>
  );

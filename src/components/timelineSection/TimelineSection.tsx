...
import {SECONDS_TO_REM_CONSTANT} from "@/components/timelineSection/parameters";
import {useScreenDetector} from "@/components/ui/use-screen-detector";
import {AddCaptionHoverButton} from "@/components/timelineSection/timeTracker/AddCaptionHoverButton";
...
const TimelineSection = ({
	...
	const [hoverRef, hovered] = useHover({delay: 500});
	const [timeTrackerCurrentTime,setTimeTrackerCurrentTime] = useState(0);
	const [hoveredAddCaptionButtonPosition, setHoveredAddCaptionButtonPosition] = React.useState(0);
	const IGNORE_SCROLL_DIFF_RANGE = window.innerWidth/1.5;
	const [displayHoverButton, setDisplayHoverButton] = useState(false);


	const updateAddCaptionHoverButtonPosition = (e: any) => {
		...
		if (hovered && (Math.abs(timeTrackerCurrentTime - currentTime) < 0.1)) {
	...
	const updateTimeTrackerPosition = (e: any) => {
		...
		const newPosition = (percentage * videoDuration*15*16);
		setHoveredAddCaptionButtonPosition(newPosition)
		setTimeTrackerCurrentTime(percentage * videoDuration);
	};
			...
			<aside className="TimelineSection h-full" >
				...
				>
					<div className="Content">
						<header
							className="h-1/4 flex items-center cursor-pointer justify-between px-4 text-gray border-b-red pb-14 relative"
							style={{width}} // Dynamically apply calculated width
							onClick={updateTimeTrackerPosition}
							onMouseMove={updateAddCaptionHoverButtonPosition}
							ref={hoverRef}
						>
							<div style={{left: `${hoveredAddCaptionButtonPosition - 18}px`}} className="absolute bottom-5 z-50">
								<AddCaptionHoverButton hovered={displayHoverButton} timeTrackerCurrentTime={timeTrackerCurrentTime}/>
							</div>
